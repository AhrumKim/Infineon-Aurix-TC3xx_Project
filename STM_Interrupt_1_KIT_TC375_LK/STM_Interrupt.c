...
#define TIMER_INT_TIME          100                             /* Time between interrupts in ms                    */

#define LED1                    &MODULE_P00,5                   /* LED toggled in Interrupt Service Routine (ISR)   */
...
Ifx_TickTime g_ticksFor100ms;                                   /* Variable to store the number of ticks to wait    */
Ifx_TickTime g_ticksFor200ms;
...
void isrSTM(void)
{
    ...
    IfxStm_increaseCompare(STM, g_STMConf.comparator, g_ticksFor100ms);
...
void initLED(void)
{
    ...
    IfxPort_setPinMode(LED2, IfxPort_Mode_outputPushPullGeneral);    /* Initialize LED port pin                      */
    IfxPort_setPinState(LED2, IfxPort_State_high);                   /* Turn off LED (LED is low-level active)       */
}
...
void initSTM(void)
{
    ...
    g_STMConf.ticks = g_ticksFor100ms;              /* Set the number of ticks after which the timer triggers an
                                                     * interrupt for the first time                                 */
            /* Set the number of ticks after which the timer triggers an
...
void initPeripherals(void)
{
    ...
    g_ticksFor100ms = IfxStm_getTicksFromMilliseconds(BSP_DEFAULT_TIMER, TIMER_INT_TIME);
                                       /* Initialize the port pin to which the LED is connected        */
